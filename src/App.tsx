    ...
    <div className="min-h-screen bg-[radial-gradient(circle_at_top,_#153725_0%,_#08110c_45%,_#040706_100%)] text-zinc-100">
      ...
      <header className="sticky top-0 z-50 border-b border-white/10 bg-black/30 backdrop-blur-xl">
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4">
          ...
          <div className="flex items-center gap-2"><IconButton icon={<History className="h-5 w-5" />} /><IconButton icon={<Settings className="h-5 w-5" />} /></div>
        </div>
      </header>
      ...
      <main className="relative mx-auto grid max-w-7xl gap-8 px-4 py-8 lg:grid-cols-12">
        <section className="flex flex-col gap-6 lg:col-span-8">
          ...
          <button type="button" onClick={() => { setShowStatsView(true); setTable((current) => endSessionEarly(current)); }} disabled={table.phase === 'round-over'} className={`self-center rounded-2xl px-8 py-3 text-sm font-black tracking-[0.24em] transition ${table.phase !== 'round-over' ? 'border border-rose-400/30 bg-rose-500/10 text-rose-200 hover:bg-rose-500/20' : 'cursor-not-allowed bg-white/10 text-zinc-500'}`}>조기 종료하기</button>
