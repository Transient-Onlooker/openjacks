...
type UISettings = {autoOpenStats: boolean; bankruptcyAlert: boolean; confirmEarlyExit: boolean};
...
const DEFAULT_SETTINGS: UISettings = {autoOpenStats: true, bankruptcyAlert: true, confirmEarlyExit: true};
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (settings.autoOpenStats) {
      setShowStatsView(true);
    }
  }, [currentRoundNumber, roundNet, settings.autoOpenStats, table]);
    ...
    <div className="min-h-screen bg-[radial-gradient(circle_at_top,_#153725_0%,_#08110c_45%,_#040706_100%)] text-zinc-100">
      ...
      <header className="sticky top-0 z-50 border-b border-white/10 bg-black/30 backdrop-blur-xl">
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4">
          <div className="flex items-center gap-3">
            ...
            <div><p className="text-[10px] uppercase tracking-[0.35em] text-emerald-300/80">확률 실습 테이블</p><h1 className="text-lg font-black uppercase tracking-[0.2em] text-white">Openjacks</h1></div>
          </div>
      ...
      <main className="relative mx-auto grid max-w-7xl gap-8 px-4 py-8 lg:grid-cols-12">
        <section className="flex flex-col gap-6 lg:col-span-8">
          <div className="relative overflow-hidden rounded-[2rem] border border-emerald-400/20 bg-[#0d2c1f] p-4 shadow-[0_30px_120px_rgba(0,0,0,0.45)] sm:p-8">
            ...
            <div className="relative flex min-h-[34rem] flex-col justify-between gap-8 rounded-[1.6rem] border border-white/8 px-4 py-8 sm:px-8">
              {table.phase === 'round-over' ? <RoundOver table={table} onReplay={() => { setShowStatsView(false); setTable((current) => prepareNextRound(current)); }} onStats={() => setShowStatsView(true)} /> : null}
          ...
          <button type="button" onClick={() => { if (settings.confirmEarlyExit && !window.confirm('현재 세션을 조기 종료할까요? 진행 중 베팅은 환불됩니다.')) return; const ended = endSessionEarly(table); pushHistory(setHistoryEntries, ended, currentRoundNumber, 0); setShowStatsView(settings.autoOpenStats); setTable(ended); }} disabled={table.phase === 'round-over'} className={`self-center rounded-2xl px-8 py-3 text-sm font-black tracking-[0.24em] transition ${table.phase !== 'round-over' ? 'border border-rose-400/30 bg-rose-500/10 text-rose-200 hover:bg-rose-500/20' : 'cursor-not-allowed bg-white/10 text-zinc-500'}`}>조기 종료하기</button>
        ...
        <aside className="flex flex-col gap-6 lg:col-span-4">
          ...
          <Panel title="기본 전략" icon={<ShieldCheck className="h-5 w-5 text-emerald-400" />} badge="실시간"><div className="rounded-2xl border border-emerald-400/20 bg-emerald-500/8 p-4"><p className="text-[11px] font-bold uppercase tracking-[0.35em] text-emerald-300">추천 액션</p><p className="mt-2 text-3xl font-black uppercase tracking-[0.18em] text-white">{recommendation ? actionLabel(recommendation.action) : '대기'}</p><p className="mt-3 text-sm leading-6 text-zinc-300">{recommendation?.reason ?? '라운드를 시작하면 현재 손패 기준의 추천 액션을 보여줍니다.'}</p></div><div className="grid grid-cols-2 gap-3"><SmallMetric label="안전 확률" value={analysis ? percent(analysis.safeProbability) : '-'} color="text-emerald-300" /><SmallMetric label="버스트 확률" value={analysis ? percent(analysis.bustProbability) : '-'} color="text-zinc-100" /></div></Panel>
